...
namespace Izi.Travel.Data.DbVersion.Updaters.Base
{
  public abstract class DbUpdaterBase : IDbUpdater
  {
    ...
    protected abstract DataContext CreateContext();
        ...
        // Replace all instances of "using (DataContext context = CreateContext())" with a try/finally pattern
        public virtual void Drop()
        {
            ...
            {
                // If DataContext implements IDisposable in the future, dispose here.
        ...
        public virtual void Update()
        {
            ...
            try
            {
                if (!context.DatabaseExists())
                {
                    ...
                    databaseSchemaUpdater.DatabaseSchemaVersion = profiles == null ||
                        !((IEnumerable<IDbUpdaterProfile>)profiles).Any<IDbUpdaterProfile>() ? 1
                        : ((IEnumerable<IDbUpdaterProfile>)profiles).Max<IDbUpdaterProfile>((Func<IDbUpdaterProfile, int>)(x => x.DbVersion));
                    databaseSchemaUpdater.Execute();
                }
                else
                {
                    int dbVersion = 0; // context.CreateDatabaseSchemaUpdater().DatabaseSchemaVersion;
                    IOrderedEnumerable<IDbUpdaterProfile> orderedEnumerable = profiles != null
                        ? ((IEnumerable<IDbUpdaterProfile>)profiles).Where<IDbUpdaterProfile>((Func<IDbUpdaterProfile, bool>)
                        (x => x.DbVersion > dbVersion)).OrderBy<IDbUpdaterProfile, int>((Func<IDbUpdaterProfile, int>)(x => x.DbVersion)) : (IOrderedEnumerable<IDbUpdaterProfile>)null;
