...
namespace Weakly
{
  ...
  public static class DynamicProperty
  {
    ...
    private static Func<object, object> CompileGetter(PropertyInfo property)
    {
      ...
      return Expression.Lambda<Func<object, object>>((Expression) Expression.Convert((Expression) Expression.Property((Expression) Expression.Convert((Expression) parameterExpression, property.DeclaringType), property), typeof (object)), parameterExpression).Compile();
    }
    ...
    private static Action<object, object> CompileSetter(PropertyInfo property)
    {
      ...
      UnaryExpression unaryExpression = Expression.Convert((Expression) parameterExpression3, property.DeclaringType);
      UnaryExpression right = Expression.Convert((Expression) parameterExpression4, property.PropertyType);
      return ((Expression<Action<object, object>>) ((parameterExpression1, parameterExpression2) => Expression.Assign((Expression) Expression.Property((Expression) unaryExpression, property), (Expression) right))).Compile();
    }
  }
}
