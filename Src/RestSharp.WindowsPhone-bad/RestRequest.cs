        ...
        if (includedProperties.Length == 0 || includedProperties.Length > 0 && ((IEnumerable<string>) includedProperties).Contains<string>(property.Name))
        {
          ...
          if (source != null)
          {
            if (propertyType.IsArray)
            {
              ...
              source = ((Array) source).Length <= 0 || elementType == null 
            || /*!elementType.IsPrimitive && !elementType.IsValueType && */
            elementType != typeof (string) ? (object) string.Join(",", (string[]) source) : (object) string.Join(",", ((Array) source).Cast<object>().Select<object, string>((Func<object, string>) (item => item.ToString())).ToArray<string>());
            }
