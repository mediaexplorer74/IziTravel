  ...
  public class CustomLogger : ILog
  {
    ...
    private void Log(string level, string message)
    {
      if (this._isSuppressed)
        return;
      string data = string.Format("{0} [{1}] {2}: {3}", (object) DateTime.Now.ToString("G",
          (IFormatProvider) CustomLogger.CultureInfo), (object) this._typeFullName,
          (object) level, (object) message);
    ...
    public static string GetLog()
    {
      try
      {
        ...
        using (IsolatedStorageFile storage = IsolatedStorageFile.GetUserStoreForApplication())
          ((IEnumerable<string>) storage.GetFileNames(CustomLogger.GetPath("*")))
                        .OrderBy<string, string>((Func<string, string>)
                        (x => x)).ToList<string>().ForEach((Action<string>)
                        (fileName => stringBuilder.Append(CustomLogger.Read(storage, fileName))));
      ...
      try
      {
        using (IsolatedStorageFile storage = IsolatedStorageFile.GetUserStoreForApplication())
        {
          if (storage.DirectoryExists("Logs"))
            ((IEnumerable<string>) storage.GetFileNames(CustomLogger.GetPath("*")))
                            .OrderByDescending<string, string>((Func<string, string>)
                            (x => x)).Skip<string>(3).ToList<string>().ForEach((Action<string>) 
                            (fileName => CustomLogger.Delete(storage, fileName)));
          else
    ...
    {
      using (IsolatedStorageFileStream storageFileStream = storage.OpenFile(
          CustomLogger.GetPath(fileName), FileMode.OpenOrCreate, 
    ...
    private static string Read(IsolatedStorageFile storage, string fileName)
    {
      using (IsolatedStorageFileStream storageFileStream 
                = storage.OpenFile(CustomLogger.GetPath(fileName), 
