...
namespace Izi.Travel.Toolkit.Controls.Maps
{
  public static class QueryExtensions
  {
    ...
    {
            return (Task<IList<MapLocation>>)geocodeQuery.QueryAsync(default,default); // <IList<MapLocation>>
    }

    public static Task<IList<MapLocation>> GetMapLocationsAsync(
      this ReverseGeocodeQuery reverseGeocodeQuery)
    {
            return (Task<IList<MapLocation>>)reverseGeocodeQuery.QueryAsync(default, default); //reverseGeocodeQuery.QueryAsync<IList<MapLocation>>();
        }

    public static Task<Route> GetRouteAsync(this RouteQuery routeQuery)
    {
            return routeQuery.QueryAsync();//.QueryAsync<Route>();
    }

    private static Task<TResult> QueryAsync<TResult>(this Query<TResult> query)
    {
      EventHandler<QueryCompletedEventArgs<TResult>> queryCompletedHandler
                = (EventHandler<QueryCompletedEventArgs<TResult>>) null;
      
      TaskCompletionSource<TResult> taskCompletionSource
                = QueryExtensions.CreateSource<TResult>((object) query);

      queryCompletedHandler 
                = (EventHandler<QueryCompletedEventArgs<TResult>>)
                ((sender, e) => QueryExtensions.TransferCompletion<TResult>(
                    taskCompletionSource, (AsyncCompletedEventArgs) 
                    e, (Func<TResult>) (() => e.Result), (Action) (
