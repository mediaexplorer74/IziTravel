// ********************************************************************
// Type: Izi.Travel.Shell.Toolkit.Controls.FlipView
...
namespace Izi.Travel.Shell.Toolkit.Controls
{
  ...
  public class FlipView : /*TemplatedItemsControl<FlipViewItem>,*/  ItemsControl //, ISupportInitialize
  {
    ...
    private bool _supressHandleManipulation;
        ...
        public static readonly DependencyProperty SelectedIndexProperty = DependencyProperty.Register(nameof(SelectedIndex), typeof(int),
            typeof(FlipView), new PropertyMetadata((object)-1,
                (PropertyChangedCallback)
                ((d, e) =>
                {
                    if (d is FlipView flipView)
                    {
                        flipView.OnSelectedIndexChanged((int)e.OldValue, (int)e.NewValue);
                    }
                })));
        ...
        public static readonly DependencyProperty SelectedItemProperty = DependencyProperty.Register(nameof(SelectedItem), typeof(object),
                typeof(FlipView), new PropertyMetadata((PropertyChangedCallback)
                ((d, e) =>
                {
                    if (d is FlipView flipView)
                    {
                        flipView.OnSelectedItemChanged(e.OldValue, e.NewValue);
                    }
                })));

        public static readonly DependencyProperty UseTouchAnimationsForAllNavigationProperty 
            = DependencyProperty.Register(nameof (UseTouchAnimationsForAllNavigation),
                typeof (bool), typeof (FlipView), new PropertyMetadata((object) true));

    public static readonly DependencyProperty UpdateSelectionModeProperty
            = DependencyProperty.Register(nameof (UpdateSelectionMode), 
                typeof (UpdateSelectionMode), typeof (FlipView), (PropertyMetadata) null);

    internal static readonly DependencyProperty IsSelectedProperty 
            = DependencyProperty.RegisterAttached("IsSelected", typeof (bool), 
                typeof (FlipView), new PropertyMetadata(
                    new PropertyChangedCallback(FlipView.OnIsSelectedChanged)));

    public static readonly DependencyProperty IsLockedProperty 
            = DependencyProperty.Register(nameof (IsLocked), typeof (bool), 
                typeof (FlipView), new PropertyMetadata((object) false));
    ...
    {
      get => (bool) this.GetValue(FlipView.IsLockedProperty);
      set => this.SetValue(FlipView.IsLockedProperty, (object) value);
    }
    ...
    internal void NotifyItemSelected(FlipViewItem container, bool isSelected)
    {
      ...
      else
      {
        ...
        {
            this.OnSelectionChanged(new SelectionChangedEventArgs((IList<object>)unselectedItems, (IList<object>)selectedItems));
        }
      ...
      object newSelectedItem)
    {
      ...
      if (num1 == 0 && !flag1)
        return;
      int num2;
      if (((!this._loaded || !this.UseTouchAnimationsForAllNavigation || this.Items.Count <= 1 || oldSelectedIndex == -1 
                ? 0
                : (Math.Abs(num1) == 1 ? 1 : 0)) & (flag1 ? 1 : 0)) != 0)
      {
...
label_7:
    ...
    {
      this.ItemsHost = this.ElementItemsPresenter.GetFirstLogicalChildByType<Panel>(false);
      if (this.ItemsHost == null)
        return;
      this.UpdateItemsHostSize();
      this.ItemsHost.SizeChanged += new SizeChangedEventHandler(this.OnItemsHostSizeChanged);
      this.Orientation = this.ItemsHost is VirtualizingStackPanel itemsHost ? itemsHost.Orientation : throw new InvalidOperationException();
      this.ScrollSelectionIntoView();
    }
    ...
    /*internal void OnManipulationDelta(object sender, ManipulationDeltaEventArgs e)
    {
      ...
      {
        double num = FlipView.AngleFromVector(e.CumulativeManipulation.Translation.X, e.CumulativeManipulation.Translation.Y) % 180.0;
    ...
    internal void OnManipulationCompleted(object sender, ManipulationCompletedEventArgs e)
    {
      ...
      if (this._hasDragDelta)
      {
        /*if (e.IsInertial)
        {
          ...
          linearVelocity = e.FinalVelocities.LinearVelocity;
          double y = linearVelocity.Y;
          double angle = FlipView.AngleFromVector(x, y);
    ...
    private void Flick(double angle)
    {
     ...
     /*private bool HasDragDelta(System.Windows.Input.ManipulationDeltaEventArgs e)
     {
    ...
    private void ReleaseMouseCaptureAtGestureOrigin()
    {
      ...
      {
        //RnD
        //foreach (UIElement inHostCoordinate  in VisualTreeHelper.FindElementsInHostCoordinates(target.TransformToVisual((UIElement) null).Transform(this._gestureOrigin), 
    ...
    {
      private static readonly PropertyPath TranslateXPropertyPath = new PropertyPath( CompositeTransform.TranslateXProperty.ToString());
      private static readonly PropertyPath TranslateYPropertyPath = new PropertyPath(CompositeTransform.TranslateYProperty.ToString());
        ...
        public Animator(CompositeTransform compositeTransform, Orientation orientation)
      {
        ...
        this._orientation = orientation;
            ...
            this._sbRunning.Completed += new EventHandler<object>(this.OnCompleted);
        ...
        Storyboard.SetTargetProperty((Timeline) this._daRunning, 
            this._orientation == Orientation.Horizontal 
              ?  FlipView.Animator.TranslateXPropertyPath.ToString() 
              : FlipView.Animator.TranslateYPropertyPath.ToString() );
      }
      ...
      {
        this.GoTo(targetOffset, duration, /*null,*/ (Action) null);
      }
