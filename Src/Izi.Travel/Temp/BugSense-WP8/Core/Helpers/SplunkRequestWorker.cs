...
// Type: BugSense.Core.Helpers.SplunkRequestWorker
...
{
  internal class SplunkRequestWorker : IRequestWorker
  {
    ...
    public async Task<BugSenseResponseResult> SendEventAsync(BugSenseEventTag tag)
    {
      ...
      if (BugSenseProperties.HandleWhileDebugging)
      {
        ...
        {
          string url = BugSenseProperties.CreateUrl(0, 1);
          result = await this.ServiceRepository.ExecuteBugSenseRequestAsync(url, bugSenseEventData, false, this.ContentResolver.EventContentType());
      ...
      string filePath = null)
    {
      ...
      if (BugSenseProperties.HandleWhileDebugging)
      {
        ...
        if (logResult.ResultState == BugSenseResultState.OK)
        {
          string url = BugSenseProperties.CreateUrl(1, 0);
          result = await this.ServiceRepository.ExecuteBugSenseRequestAsync(url, serializedData, true, this.ContentResolver.ErrorContentType());
          result.ClientRequest = jsonSerializeResult.DecodedJson;
          if (result.ResultState == BugSenseResultState.OK)
          {
    ...
    public async Task ProcessPreviousLoggedCrashesAsync()
    {
      ...
      string massContent = string.Empty;
      if (filesToSend == null || filesToSend.Count <= 0)
        return;
      string massDecodedContent = string.Empty;
      bool isError = false;
      int errorsCount = 0;
      int eventsCount = 0;
      foreach (string fileName in filesToSend)
      {
        ...
        isError = filePath.Contains("LLC") || filePath.Contains("CCC");
        if (isError)
          ++errorsCount;
        else
          ++eventsCount;
        ...
        (^this).\u003CmassContent\u003E5__70 += content;
        // ISSUE: explicit reference operation
        // ISSUE: reference to a compiler-generated field
        (^this).\u003CmassDecodedContent\u003E5__71 += this.RequestJsonSerializer.DecodeEncodedCrashJson(content);
      }
      string url = BugSenseProperties.CreateUrl(errorsCount, eventsCount);
      BugSenseResponseResult result = await this.ServiceRepository.ExecuteBugSenseRequestAsync(url, massContent, isError, this.ContentResolver.ErrorContentType());
      if (result.ResultState == BugSenseResultState.OK)
      {
        foreach (string fileName in filesToSend)
        {
      ...
      result.ClientRequest = massDecodedContent;
