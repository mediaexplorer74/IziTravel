...
namespace RestSharp
{
  ...
  public class RestClient : IRestClient
  {
      ...
      CancellationToken token)
    {
      ...
      try
      {
        RestRequestAsyncHandle async = this.ExecuteAsync<T>(request, (Action<IRestResponse<T>, RestRequestAsyncHandle>) ((response, _) =>
        {
    ...
    public RestClient(string baseUrl)
      : this()
    {
      this.BaseUrl = !string.IsNullOrEmpty(baseUrl) ? new Uri(baseUrl) : throw new ArgumentNullException(nameof (baseUrl));
    }
    ...
    public Uri BuildUri(IRestRequest request)
    {
      ...
      IEnumerable<Parameter> parameters1 = request.Parameters.Where<Parameter>((Func<Parameter, bool>) (p => p.Type == ParameterType.UrlSegment));
      UriBuilder uriBuilder = new UriBuilder(this.BaseUrl);
      foreach (Parameter parameter in parameters1)
      {
        if (parameter.Value == null)
          throw new ArgumentException(string.Format("Cannot build uri when url segment parameter '{0}' value is null.", (object) parameter.Name), nameof (request));
      ...
      if (this.BaseUrl != (Uri) null && !string.IsNullOrEmpty(this.BaseUrl.AbsoluteUri))
      {
        if (!this.BaseUrl.AbsoluteUri.EndsWith("/") && !string.IsNullOrEmpty(uriString))
          uriString = "/" + uriString;
        uriString = string.IsNullOrEmpty(uriString) ? this.BaseUrl.AbsoluteUri : string.Format("{0}{1}", (object) this.BaseUrl, (object) uriString);
      }
      IEnumerable<Parameter> parameters2 = request.Method == Method.POST || request.Method == Method.PUT || request.Method == Method.PATCH ? (IEnumerable<Parameter>) request.Parameters.Where<Parameter>((Func<Parameter, bool>) (p => p.Type == ParameterType.QueryString)).ToList<Parameter>() : (IEnumerable<Parameter>) request.Parameters.Where<Parameter>((Func<Parameter, bool>) (p => p.Type == ParameterType.GetOrPost || p.Type == ParameterType.QueryString)).ToList<Parameter>();
    ...
    {
      return string.Join("&", parameters.Select<Parameter, string>(new Func<Parameter, string>(RestClient.EncodeParameter)).ToArray<string>());
    }

    private static string EncodeParameter(Parameter parameter)
    {
      return parameter.Value != null ? parameter.Name.UrlEncode() + "=" + parameter.Value.ToString().UrlEncode() : parameter.Name.UrlEncode() + "=";
    }
    ...
    private void ConfigureHttp(IRestRequest request, IHttp http)
    {
      ...
      http.UserAgent = input.HasValue() ? input : "RestSharp/" + (object) RestClient.version;
