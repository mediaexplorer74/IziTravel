...
namespace RestSharp
{
  public static class RestClientExtensions
  {
    ...
    {
      return client.ExecuteGetTaskAsync<T>(request).ContinueWith<T>((Func<Task<IRestResponse<T>>, T>) (x => x.Result.Data));
    }

    public static Task<T> PostTaskAsync<T>(this IRestClient client, IRestRequest request) where T : new()
    {
      return client.ExecutePostTaskAsync<T>(request).ContinueWith<T>((Func<Task<IRestResponse<T>>, T>) (x => x.Result.Data));
    }

    public static Task<T> PutTaskAsync<T>(this IRestClient client, IRestRequest request) where T : new()
    {
      request.Method = Method.PUT;
      return client.ExecuteTaskAsync<T>(request).ContinueWith<T>((Func<Task<IRestResponse<T>>, T>) (x => x.Result.Data));
    }
    ...
    {
      Parameter parameter = restClient.DefaultParameters.SingleOrDefault<Parameter>((Func<Parameter, bool>) (p => p.Name.Equals(name, StringComparison.OrdinalIgnoreCase)));
      if (parameter == null)
        return;
